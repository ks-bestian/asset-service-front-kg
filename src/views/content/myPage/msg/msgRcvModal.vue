...
const fnSearchUser = () => {
    ...
    store.API_LIST('myPage/msg/user', param).then((data) => {
        userList.value = data.data.data.filter(item => item.userId !== store.loginInfo.userId)
    }).catch(({ message }) => {
        console.log(message)
    })
}
...
const filteredData = computed(() => {
    if (selectTab.value === 'cmit') {
        return userList.value; //TODO Committee Staff? Humor?

    } else if( selectTab.value === 'member') {
...
const fnSearchDept = () => {
    ...
    store.API_LIST(apiUrl, params).then((data) => {
        ...
        cmitList.value.push(...data.data.data.filter(data => data.uprDeptCd == 'CMT'));
    }).catch(({ message }) => {
            ...
            <div class="popup_inner">
                <div class="popup_body">
                    <div class="popup_cont">
                        <div class="form_col type4 mb_2">
                            <nav class="tab_menu type2">
                                <ul class="tab_list">
                                    <li class="on"><a href="javascript:void(0)" @click="fnTab('member')">{{ t('10344') }}</a></li><!--member-->
                                    <li><a href="javascript:void(0)" @click="fnTab('cmit')">{{ t('10045') }}</a></li><!--committee-->
                                    <li><a href="javascript:void(0)" @click="fnTab('user')">{{ t('10345') }}</a></li><!--employee-->
                                </ul>
                            </nav>
                            ...
                            <div class="board_search">
                                <div class="search_inner">
                                    <div class="form_row">
                                        ...
                                        <div class="input_item" v-else-if="selectTab === 'cmit'">
                                            <label class="form_label">{{ t('10045') }}</label><!--committee-->
                                            <select class="form_control" v-model="cmitCd">
                                                <option value="">{{ t('10224') }}</option><!--Please select.-->
                                                <option v-for="(item, i) in cmitList" :key="i" :value="item.deptCd">{{
                                                    item.deptNm }}
                                                </option>
                                            </select>
                                        </div>
                                ...
                                <div class="search_btn">
                                    <button type="button" class="v_btn btn_primary btn_sm" @click="fnSearchUser"><i class="v_ico ico_search_white"></i><span>{{ t('10053') }}</span></button><!--check-->
                                </div>
                            </div>
                            ...
                            <div class="board_info">
                                <div class="left">
                                    <div class="total_num">Total <span class="text_primary">{{ filteredData.length }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="v_table table_list mb_1" v-if="selectTab === 'member'">
                                <DataTable :value="filteredData" scrollable scrollHeight="320px"
                                    ...
                                    <Column field="userNm" :header="t('10152')" class="text_left" sortable></Column><!--name-->
                            ...
                            <div class="v_table table_list mb_1" v-else-if="selectTab === 'cmit'">
                                <DataTable :value="filteredData" scrollable scrollHeight="320px" data-key="userId"
                                    v-model:selection="selectUsers">
                                    <Column field="userId" selection-mode="multiple" style="width: 5%;"
                                        class="text_center">
                                    ...
                                    <Column field="userNm" :header="t('10152')" class="text_left" sortable></Column>
                                    <Column field="cmitNm" :header="t('10045')" class="text_left">
                                        <template #body="{data}">
                                            {{ data.cmitNm}} 
                                            {{ (data.deptCd?.startsWith('CMT') && data.deptCd !== 'CMT') ? '/ '+ data.deptNm : '' }}
                                        </template>
                                    </Column><!--committee-->
                                    <template #empty>
                                        <div class="no_data">
                    ...
                    <div class="popup_footer">
                        <div class="btn_group">
                            <button type="button" class="v_btn btn_primary btn_md" @click="fnAddRcv">{{ t('10089') }}</button><!--check-->
                            <button type="button" class="v_btn btn_outline_primary btn_md"
                                @click="emit('close')">{{ t('10163') }}</button><!--cancellation-->
                        </div>
                    </div>
                </div>
            </div>
