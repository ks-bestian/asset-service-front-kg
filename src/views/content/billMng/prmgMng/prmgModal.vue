...
const prmgNo = ref(props.review?.prmgNo);

const fileUploads = ref([]);

const FILE_KIND_CD = '280'; // Legal action
...
const fnSave = () => {
	...
	formData.append("prmgDt", rsltDt.value.replace(/-/g, ''));
	formData.append("prmgNo", prmgNo.value);
	// formdata.append ("RMRKKKG", RMRKKKG.VALUE);
	// formdata.append ("rmrkru", rmrkru.value);

	formData.append("clsCd","320"); //horror
	...
	const apiUrl = '/bill/review/billMng/prmg';
...
const resolver = () => {
	...
	if(!prmgNo.value) {
		errors.prmgNo = [{ message: 'required.' }];
	}
						...
						<div class="v_table">
							<table>
								...
								<tbody>
									...
									<tr>
										<th scope="row">{{ t('10562') }}<span class="text_red">*</span></th>
										<td colspan="3">
											<FormField v-slot="$field" name="prmgNo" initialValue="" style="flex-grow: 1;">
												<input type="text" class="form_control" v-model="prmgNo">
												<Message v-if="$field?.invalid" severity="error" size="large" variant="simple">{{ $field.error?.message }}</Message>
											</FormField>
										</td>
									</tr>
