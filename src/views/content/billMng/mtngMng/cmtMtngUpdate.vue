...
const fnSave = (event) => {
    ...
    confirm.require({
        ...
        accept: () => {
            ...
            params.mtngPlc = mtngData.value.mtngPlc;
            params.dueDtm = replace(mtngData.value.dueDate, /-/g, '')+mtngData.value.dueHour+mtngData.value.dueMin;
            params.attendantList = participantList.value;
            params.agendaList = agendaList.value;
            const apiUrl = `/bill/mtng/from/update`;
...
const fnGetMtngData = () => {
    ...
    store.API_LIST(apiUrl, params).then((data) => {
      ...
      mtngData.value.dueDate = dayjs(mtngData.value.dueDtm).format('YYYY-MM-DD');
      mtngData.value.dueHour = dayjs(mtngData.value.dueDtm).format('HH');
      mtngData.value.dueMin = dayjs(mtngData.value.dueDtm).format('mm');
								...
								<td>
									<div class="input_group">
										<FormField v-slot="$field" name="dueDate" initialValue="" style="flex-grow: 1;">
											<input type="date" class="form_control"  v-model="mtngData.dueDate" style="width: 40%;">
