...
const mtngTypeCds = store.getComCodes('1014') //Meeting

const searchMtngTypeCd = ref('');
...
const fnGetMtngList = () => {
	...
	params.mtngTypeCd = searchMtngTypeCd.value; //Meeting
...
const fnGoDetail= (e)=>{
    router.push({name: 'mtng.allMtng.detail', query : {  mtngId: e.data.mtngId }});
}
...
const fnExcelDownload = () => {
    ...
    const title = t('10132') + t('10084')//'List of meeting information';


    tableDatas.value.map(item => {
        item.openDtm = dayjs(item.openDtm).isValid ? dayjs(item.openDtm).format('DD/MM/YYYY') : '';
        item.dueDtm = dayjs(item.dueDtm).isValid ? dayjs(item.dueDtm).format('DD/MM/YYYY HH:MM') : '';
		item.mtngTypeCd = item.mtngTypeNm;
    })
						...
						<div class="content_section">
							<div class="board_search">
								<div class="search_inner">
									...
									<div class="form_row">
										<div class="input_item">
											<label class="form_label">{{ t('10181') }}</label>
											<select class="form_control" v-model="searchMtngTypeCd">
												<option value="">{{ t('10066') }}</option>
												<option v-for="mtngTypeCd in mtngTypeCds" :value="mtngTypeCd.codeId">{{ mtngTypeCd.codeNm }}</option>
											</select>
										</div>
										<div class="input_item">
							...
							<div class="v_table table_list">
								...
								paginatorTemplate="RowsPerPageDropdown FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink">
								    <Column field="ageCd" :header="t('10116')" style="width: 5%;" bodyStyle="text-align: center;" sortable></Column>
								    <Column field="dueDtm" :header="t('10185')" style="width: 15%;"  bodyStyle="text-align: center;" sortable>
										<template #body="slotProps">
											{{ dayjs(slotProps.data.dueDtm, "YYYYMMDDHHmm").format("DD/MM/YYYY HH:mm") }}
										</template>
                                    </Column>
									<Column field="openDtm" :header="t('10133')" style="width: 15%;"  bodyStyle="text-align: center;" sortable>
										<template #body="slotProps">
