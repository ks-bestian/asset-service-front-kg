...
const authBtnSendLegalAct = ref(false);//Legal behavior transmission
...
const fnAuthSetup = () => {
	...
	else if(mtngData.value.statCd == '2') {
		authBtnSendLegalAct.value = true;
	}
};
...
const fnGoMtngResult = () => {
	router.push({name: 'mtng.mainMtng2.result', query : {  mtngId: mtngId }});
};
...
const fnGoUpdate = () => {
	router.push({name: 'mtng.mainMtng.update', query : { mtngId: mtngId }});
}
	...
	<div class="content_inner">
				...
				<table>
					...
					<tbody>
						<tr>
							...
							<td colspan="3">{{ mtngData.mtngTypeNm }}</td>
						</tr>
						...
						<tr>
							<th scope="row">{{ t('10116') }}</th>
							<td>{{ mtngData.ageCd }}</td>
						...
						<tr  v-if="mtngData.statCd == '2'">
							...
							<td>{{ dayjs(mtngData.openDtm).format('DD/MM/YYYY') }}&nbsp;{{ dayjs(mtngData.openDtm).format('HH:mm') }}~{{ dayjs(mtngData.closeDtm).format('HH:mm') }}</td>
							<th scope="row">{{ t('10187') }}</th>
							<td>        
								{{ mtngData.mtngPlc }}
							</td>
				...
				<div class="v_table" style="height: 10px;"> 
					...
					<div class="v_table"> 
						<DataTable :value="agendaList" >
							...
							<Column field="currentStepId" header="stepId" class="text_center" style="width: 10%;"></Column>
							<!-- <Column field="statNm" :header="t('10056')" class="text_center" style="width: 10%;"></Column> -->
						</DataTable>
		...
		<div class="btn_group_fixed ">
			<div class="btns">
				...
				<a href="javascript:void(0)" v-if="authBtnCancel" class="v_btn btn_primary btn_md" @click="fnDelete">{{ t('10173') }}</a> -->
				<a href="javascript:void(0)" v-if="authBtnReport" class="v_btn btn_primary btn_md" @click="fnGoMtngResult">{{ t('10218') }}</a>
				<!-- <a href="javascript:void(0)" v-if="authBtnReport" class="v_btn btn_primary btn_md" @click="fnSubmitMtngAgenda">{{ t('10215') }}</a> -->
				<a href="javascript:void(0)" v-if="authBtnSendLegalAct" class="v_btn btn_primary btn_md" @click="fnSubmitMtngAgenda">{{ t('10557') }}</a>
			</div>
		</div>
</div>		
