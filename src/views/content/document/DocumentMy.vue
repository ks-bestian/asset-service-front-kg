...
const reloadFolder = () => {
    ...
    DocumentService.getMyFoldersAndFiles(folderId, title, regNm).then(ff => {
        documents.value = ff;
...
const onRowDblclick = (e) => {
    ...
    // else if (e.data.fileType == 'pdf') {
    //     let apiUrl = '';
    //     apiUrl = '/document/files/download/pdf/' + e.data.fileId;
    //     axios.get(apiUrl, { responseType: 'arraybuffer', params: {  } })
    //         .then(response => {
    //             console.log('response.data', response.data);
    //             var file = new Blob([response.data], {type: 'application/pdf'});
    //             var fileURL = URL.createObjectURL(file);
    //             window.open(fileURL, "pdfView", "popup");
    //         })
    //         .catch(err => {
    //             console.error(err);
    //         })
...
<template>
    <DocumentMng :my-folder-tree="folderTree" :doc-kind="'my'" :cur-folder="curFolder" :file-list="documentsFiltered"
                ...
                <Column field="" header="" style="width: 9.6%;" class="text_center">
                    <template #body="{ data }">
                        <div v-if="data.folderYn == 'Y'">
                            <i class="pi pi-share-alt" v-tooltip.bottom="{ value: t('10588'), showDelay: 300, hideDelay: 100 }" @click="shareFile(data)" v-if="data.regId == userId"></i>
                            <i class="pi pi-pencil" v-tooltip.bottom="{ value: t('10157'), showDelay: 300, hideDelay: 100 }" @click="updateFolder(data)" v-if="data.regId == userId"></i>
                            <i class="pi pi-trash" v-tooltip.bottom="{ value: t('10173'), showDelay: 300, hideDelay: 100 }" @click="e => deleteFolder(e, data)" v-if="data.regId == userId"></i>
                        </div>
                        <div v-if="data.folderYn == 'N'">
                            <i class="pi pi-share-alt" v-tooltip.bottom="{ value: t('10588'), showDelay: 300, hideDelay: 100 }" @click="shareFile(data)" v-if="data.regId == userId"></i>
                            <i class="pi pi-pencil" v-tooltip.bottom="{ value: t('10157'), showDelay: 300, hideDelay: 100 }" @click="updateFile(data)" v-if="data.regId == userId"></i>
                            <i class="pi pi-trash" v-tooltip.bottom="{ value: t('10173'), showDelay: 300, hideDelay: 100 }" @click="e => deleteFile(e, data)" v-if="data.regId == userId"></i>
                        </div>
                    </template>
                </Column>
            </DataTable>
    ...
    <NewFolderModal v-if="newFolderModal.open" :upper-folder-id="curFolder?.folderId || -1" :dialog="newFolderModal.open"
        :upper-folder-nm="curFolder?.folderNm || ROOT_FOLDER_NM" :dept-folder-yn="'N'" @reload="init" @close="newFolderModal.open = false" />
    <FileUploadModal v-if="fileUploadModal.open" :folder-id="curFolder?.folderId || -1" :dialog="fileUploadModal.open"
        :folder-nm="curFolder?.folderNm || ROOT_FOLDER_NM" :dept-file-yn="'N'" @reload="init" @close="fileUploadModal.open = false" />
